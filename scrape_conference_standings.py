"""
Scrape all NCAA D1 conference standings to get team games played for 2025-26
"""
...
for conf_id, conf_slug, conf_name in CONFERENCES:
    url = f"https://basketball.realgm.com/ncaa/conferences/{conf_slug}/{conf_id}/standings"
    print(f"Fetching {conf_name}...")
    
    try:
        html = browser.get(url)
        soup = BeautifulSoup(html, 'lxml')
        
        # Find ALL standings tables (some conferences have multiple tables)
        tables = soup.find_all('table')

        # Look for all tables with "Overall Wins" header
        standings_tables = []
        for table in tables:
