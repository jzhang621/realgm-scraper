"""
Scrape all NCAA D1 conference standings to get team games played for 2021-22
"""
...
for conf_id, conf_slug, conf_name in CONFERENCES:
    url = f"https://basketball.realgm.com/ncaa/conferences/{conf_slug}/{conf_id}/standings/2022"
    print(f"Fetching {conf_name} (2021-22)...")
        ...
        for row in standings_table.find_all('tr')[1:]:
            ...
            if len(cells) >= 8:
                ...
                try:
                    ...
                    losses = int(cells[7].get_text(strip=True))
...
output_file = 'data/processed/team_games_2022.csv'
...
if all_teams:
    ...
    print(f"  Avg games: {sum(total_games_list)/len(total_games_list):.1f}")
    print(f"  Min games: {min(total_games_list)}")
    print(f"  Max games: {max(total_games_list)}")
