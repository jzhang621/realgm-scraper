"""
Calculate player ratings for 2024-25 NCAA D1 season using RealGM data.
...
# Load team games
team_games = {}
with open('data/processed/team_games_2025.csv', 'r') as f:
...
# Load per game stats - ONLY for teams in our D1 conference standings
stats_data = []
non_d1_teams = set()
with open('data/processed/player_stats_pergame.csv', 'r') as f:
    ...
    for row in reader:
        if row['season'] == '2024-25':
            # Filter: team must be in our scraped D1 conference standings
...
# Load misc stats (double-doubles, triple-doubles) - ONLY for D1 teams
misc_data = {}
with open('data/processed/player_stats_misc.csv', 'r') as f:
    reader = csv.DictReader(f)
    for row in reader:
        if row['season'] == '2024-25' and row['team'] in team_games:
...
def get_percentile(value, sorted_list):
    """Calculate percentile (1-100) for a value in a sorted list"""
    ...
    percentile = (count / len(sorted_list)) * 100

    # Return as integer 1-100
    return max(1, min(100, int(percentile + 0.5)))
...
for row in stats_data:
    try:
        ...
        # BASE RATING (weighted percentiles)
        base_rating = (
            pts_per * 0.575 +
            ast_per * 0.100 +
            reb_per * 0.050 +
        ...
        # GAME ADJUSTMENT (penalty for missing games)
        team_total_games = team_games.get(team, 32)  # Default to 32 if team not found
        ...
        # THREE-POINT BOOST (volume × efficiency)
        ...
        # FIELD GOAL BOOST (volume × efficiency)
        ...
        # Apply floor (minimum rating of 60)
        if final_rating < 60:
            final_rating = final_rating / 1000 + 60

        # TWO-WAY PLAYER (elite scorer + good defender)
        ...
        ratings.append({
            ...
            'PTS_PER': pts_per,
            'PM3_PER': pm3_per,
            'P3PCT_PER': p3_pct_per,
            'FGM_PER': fgm_per,
            'FGPCT_PER': fg_pct_per,
            'FTPCT_PER': ft_pct_per,
            'AST_PER': ast_per,
            'REB_PER': reb_per,
            'BLK_PER': blk_per,
            'STL_PER': stl_per,
...
# Write to CSV
output_file = 'data/processed/player_ratings_2025.csv'
...
# Show top 10
print("TOP 10 PLAYERS (2024-25):")
print(f"{'Rank':<5} {'Player':<25} {'Team':<20} {'Rating':<8}")
print("-" * 70)
for i, player in enumerate(ratings[:10], 1):
    print(f"{i:<5} {player['full_name']:<25} {player['team']:<20} {player['final_rating']:<8}")
...
print(f"  Two-way players: {sum(1 for p in ratings if p['two_way'] == 'Y')} players")
print(f"{'='*70}\n")
